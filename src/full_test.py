...
if __name__ == "__main__":
    ...
    results = sa.google_search(query, "date:r", 1)
    product = sa.extract_results(results)
    links = sa.extract_links(product)
    results = aiapi.bulk_ai_search(links)

    with open('output/full_test_Google_API_Search_Results_English.json', 'w') as f:
        json.dump(product, f, indent=2)

    with open('output/full_test_OpenAI_Results_English.json', 'w') as f:
